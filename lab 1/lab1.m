...
disp(x)
disp(x);
x(1)            %display the first value
x(2)            %display the second value
%% Exercise 1
...
N = 101;
h = (7-(-3))/(N-1);
x = zeros(N,1); %allocate array

for j = 1:N
    x(j) = ((j-1)*h)-3; %create x values
end
%disp(x);

y = zeros(N,1); %allocate array

for j = 1:N
    y(j) = abs(x(j))*x(j); %create y values
end
%disp(y);
...
xlabel('x');
ylabel('f(x)');
title('Example plot of two functions computed by vectorization');
legend('x^2', 'sin(2*pi*x)','Location','SouthWest');
%% Speed Comparison
...
tic; %start timing first procedure
y = zeros(N,1);
...
clear y;
tic; %start timing second procedure
y = atan(x);
toc;
...
%   appropriately.
%
%  general solution:        y(t) = exp(-t^2) * (t + C)
%  solution to y(1) = -1:   y(t) = exp(-t^2) * (t - e - 1)
%  solution to y(1) = 0:    y(t) = exp(-t^2) * (t - 1)
%  solution to y(1) = 1:    y(t) = exp(-t^2) * (t + e - 1)
%
%% 
...
t = linspace(1, 5);                     %creates the t vector

y1 = exp(-t.^2) .* (t - exp(1) -1);     %vector for soln to y(1) = -1
y2 = exp(-t.^2) .* (t - 1);             %vector for soln to y(1) = 0
y3 = exp(-t.^2) .* (t + exp(1) -1);     %vector for soln to y(1) = 1

plot(t, y1, t, y2, t, y3);
xlabel('t');
ylabel('y(t)');
title('Plot of three particular solutions of dy/dt + (2*t)*y = 1/exp(t^2 )');
legend('exp(-t^2) *(t-e-1)', 'exp(-t^2) *(t-1)', 'exp(-t^2) *(t+e-1)','Location', 'NorthEast');
...
f = @(x) (x.^3 - abs(x)) .* log(x.^2+1);  %log() is ln() in matlab
f(0)
f(1)
f(-1)
...
guess = 0;
y = fzero(@(y)f(0, y), guess);
fprintf('y(%g) = %g\n', 0, y); %ans y(0) = 0.682328

guess = 0.5;
x = fzero(@(x)f(x, 1/2), guess);
fprintf('x(%g) = %g\n', 1/2, x); %ans: x(0.5) = 0.567406

guess = 1;
x = fzero(@(x)f(x, 1/2), guess);
fprintf('x(%g) = %g\n', 1/2, x); %ans: x(0.5) = 1.05844
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4 + 1; %added the constant +1 because without it, f(0,0) = -1 and we want it to = 0
xvals = linspace(-2, 2);
yvals = zeros(size(xvals));

for j = 1:length(xvals)

    yvals(j) = fzero(@(y) f(xvals(j), y), 0); %assign y values
...
% general implicit solution: y - exp(-sin(x) + x.^3) + c = 0
% particular solution y(0) = 1 has c = 0 as included below

f = @(x,y) -log(abs(y)) - sin(x) + x.^3; %implicit function
xvals = linspace(-1.5, 1.25);
yvals = zeros(size(xvals));

for j = 1:length(xvals)
    yvals(j) = fzero(@(y) f(xvals(j), y), 0.9); %the particular solution has a constant C = 0
end

plot(xvals, yvals);
xlabel('x');
ylabel('y');
title('Particular Solution of -ln|y|-sin(x)+x^3')

% by separable ODE, the general solution: y = exp(-sinx + x^3 + c) 
% for particular solution y(0) = 1, y + exp(-sinx + x^3) where c = 0.

%% Install iode
% Objective: install iode
% 
% Details: visit https://faculty.math.illinois.edu/iode/ and follow the instructions 
% there to download and install iode.
...
%
% Answer:
% Above the x-axis (y=0), the solutions tend upwards in a parabolic shape,
% where y values go to infinity if y > 0
% Below the x-axis, there are also parabolic shapes opening upwards, but
% their concavity changes to downwards as x -> inf and x -> -inf, which
% makes the slope go to 0 as it approaches the equilibrium y = 0
% The solution is even (has an x^2 term and constants) which means
% y(x) = y(-x) regardless of the solution. This is demonstrated by the
% symmetry of behaviour over the y axis.
%
%
%% Exercise 12
...
%
% constant solutions being approached: y= -1, 1, 2
%
% y(0) < -1:        solutions approach y=-1
% -1 < y(0) < 1:    solutions approach y=1 (two equilibriums y=-1 and y=1)
% 1 < y(0) < 2:     solutions approach y=1 (two equilibriums y=1 and y=2)
% 2 < y(0):         solutions approach infinity
%
% The value of y(0) will give it a position relative to the multiple
% solutions (equilibrium), so it determines the path one will follow to
% approach a particular solution. Solutions cannot cross, so there is a
% consistent path they will follow if y(0) is in a specific range
%
% y=-1 is a semistable equilibrium and 
% y=1 is a stable equilibrium and
% y=2 is an unstable equilibrium
%
% between y= -1 and y=1, solutions have a positive slope
% between y=1 and y=2, solutions have a negative slope, making y=1 a stable
% equilibrium
%
