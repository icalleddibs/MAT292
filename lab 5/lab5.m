...
% decay while oscillating: 100% (6/6)
%
%
% (c) Solve the DE and write the exact solution. Explain why this justifies
% your answer in (b).
% eigenvalues: -1/2 +/- 2i (complex with negative real)
% the exact solution: y = exp((-1/2)*t) * (c1*cos(2*t) + c2*(sin(2*t))
%
% the negative exponent of e means that as t approaches infinity, that term
% will go to 0. since it is multiplied by the rest of the solution, the
% whole soln goes to 0 which indicates decay. however, while t is
% approaching infinity, the cosine and sine functions will cause
% oscillatory motion. therefore, it makes sense that all the solutions will
...
%
% (b) Based on the results of (b), state what percentage of solutions 
% decay, grow, grow while oscillating, or decay while oscillating.
% decay: 0%
% grow: 100% (6/6)
% grow while oscillating: 0%
...
% (c) Solve the DE and write the exact solution. Explain why this justifies
% your answer in (b).
% eigenvalues: (-sqrt(3) +/- 2) / 2 (one positive real, one negative real)
% exact solution: y = c1*exp((-sqrt(3)+2)*t/2) + c2*exp((-sqrt(3)-2)*t/2)
%
% the second part (c2) has a negative exponent on e, meaning as t
% approaches infinity, that portion of the solution goes to 0. however, the
% first part (c1) has a positive exponent on e, meaning as t approaches
% infinity, the solution dominates the overall solution and it will tend to
% positive or negative infinity. there are no cosine or sine oscillatory
% terms. therefore, it makes sense that all the solutions will grow.
...
% (c) Solve the DE and write the exact solution. Explain why this justifies
% your answer in (b).
% eigenvalues: (-sqrt(3) +/- sqrt(2)) / 2 (two negative reals)
% exact solution: y = c1*exp((-sqrt(3)+sqrt(2))*t/2) +
% c2*exp((-sqrt(3)-sqrt(2))*t/2)
% 
% both the exponents on e are negative, meaning as t approaches infinity,
% both terms and the whole solution will go to 0. there are no cosine or
% sine oscillatory terms. therefore it makes sense that all the solutions
% will decay. (we know that sqrt(3) > sqrt(2))
...
%
% use the matlab function |roots| to solve the roots, which would be the
% eigenvalues.
p = [1, 2, 6, 2, 5];
r = roots(p);
% the eigenvalues are: -1+2i, -1-2i, 1i, -1i
% we can use the general solution for complex numbers combined with
% multiple eigenvectors to find the general solution of the 4th order ode
%
% exact solution: y = exp(-t)*(c1*cos(2*t) + c2*sin(2*t)) +
% exp(0*t)*(c3*cos(t) + c4*sin(t)) where exp(0) = 1 so:
% y = exp(-t)*(c1*cos(2*t) + c2*sin(2*t)) + c3*cos(t) + c4*sin(t)
%
...
%
% from the previous question, we know that negative exponents on e will
% make those terms go to 0 as t approaches infinity, so the c1 and c2 terms
% all go to 0. however, we have a cosine and sine term that are multiplied
% by e^0, or 1, so they will keep oscillating forever by t (added terms).
% therefore, we would predict that all the solutions are just oscillating.
%
% decay: 0%
% grow: 0% 
% grow while oscillating: 0%
...
%
% solution behaviours:
% (a) grows (has a real positive eigenvalues)
% (b) grows (has a real positive eigenvalues)
% (c) decays (has two negative real eigenvalues)
% (d) decays while oscillating (has two negative complex eigenvalues)
% (e) oscillating (real part is 0, e^0=1, leaves only sin/cos terms)
% (f) grows while oscillating (has two positive complex eigenvalues)
%
...
%
y0 = 1; %initial y condition
y1 = 0; %initial y' condition
t0 = 0;
tN = 20;
h = 0.05; %arbitrary? picked a small enough step.
t = linspace(t0, tN, tN/h);
f = @(t, y_prime, y) -exp(-t/5)*y_prime - (1-exp(-t/5))*y + sin(2*t);

[T, Y] = DE2(t0, tN, y0, y1, h, f);
...
%there are no significant differences, except maye that the IODE solution
%tends to reproduce the same curve but slightly shifted to the right.
%otherwise, both the euler method solutions are very similar.
%it will depend on the step size used. if i chose a smaller step size, it
%would look much closer, but i wanted to use a reasonable number to show
%that they are not exactly the same, just very close.
